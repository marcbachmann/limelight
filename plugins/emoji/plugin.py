...
def build_html(appearance, content):
    ...
    <script>
        function copyToClipboard(emoji) {
            command = 'printf "{{emoji}}" | LANG=en_US.UTF-8 pbcopy && osascript -e \\'display notification "Copied {{emoji}}" with title "Limelight"\\'';
            flashlight.bash(command.replace(/{{emoji}}/g, emoji));
        }

        // we need this to prevent a jump to back because
        // of the anchor in the link
        // the anchor is needed to apply the selection
        // when you tabstops to jump to the next link
        function ignoreLink () {
            return false
        }
    </script>
...
def build_emoji_html(emoji):
    html = """<a class="emoji" href="#" onclick="return ignoreLink()"><i onclick="copyToClipboard('{{icon}}')">{{icon}}</i><label onclick="copyToClipboard('{{gemoji}}')">{{alias}}</label></a>"""
...
def run(output):
    import subprocess
    command = 'printf "{{output}}" | LANG=en_US.UTF-8 pbcopy && osascript -e \'display notification "Copied {{output}}" with title "Limelight"\''.replace('{{output}}', output)

...
PluginCollection.prototype.search = function (query, callback) {
  ...
  async.map(plugins, function (p, done) {
    ...
    callback(null, {groups: groups.getGroups()})
  })
...
PluginCollection.prototype.get = function (pluginName, query, callback) {
  var plugin = _.find(this.plugins, {name: pluginName})
  if (!plugin) return callback()
  plugin.get(query, callback)
}

PluginCollection.prototype.run = function (pluginName, query, callback) {
  var plugin = _.find(this.plugins, {name: pluginName})
  if (!plugin) return callback()
  plugin.run(query, callback)
}
